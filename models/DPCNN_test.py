...
class Config(object):
    ...
    def __init__(self, dataset, embedding, use_premodel):
        ...
        self.feature_map_path = './84-85-90/transform/loss_record'  # 大税号要素特征表
        self.save_path = dataset + '/saved_dict/2input-struct-one-level-87.76/' + self.model_name + '.ckpt'  # 模型训练结果
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # 两个全连接层
        self.fc1 = nn.Linear(config.num_filters * 2, config.gather_num_classes)
        # self.fc1 = nn.Linear(config.pad_size * config.dim_model * 2, config.gather_num_classes) # transform
    ...
    def forward(self, x):
        ...
        # for encoder in self.encoders:
        #     ae = encoder(ae)
        # ae = ae.view(at.size(0), -1)

        # 连接起来
        x = torch.cat((x1, x2_2), 1)  # 连接 [batch_size, num_filters(1000)]
